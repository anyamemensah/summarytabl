...
2. Multiple response data, including binary (e.g., Unselected/Selected), ordinal (e.g., never, sometimes, often), or Likert and Likert-type (e.g., strongly disagree to strongly agree) variables
...
For example, the `depressive` dataset contains eight variables that share the same variable stem: `dep`, with each one representing a different item (such as a statement, question, or indicator) used to measure depression:

```{r}
names(depressive)
```

With the `select_tbl()` function, you can summarize these responses to see how many respondents selected each answer option for every variable.
