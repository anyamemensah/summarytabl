...
To create a contingency table for two categorical variables, use the `cat_group_tbl()` function.
...
To control whether the table is displayed in long or wide format, use the `pivot` argument.
...
To exclude values from the data and specify how missing values should be handled, use the `ignore` and `na_removal` arguments. By default, missing values are removed `listwise`, but you can set `na_removal` to `pairwise` for pairwise removal instead.
...
By default, the summary table is shown in long format. To convert it to wide format, change the `pivot` argument to `wider`.
...
Similar to `cat_group_tbl()`, you can specify which values to exclude and determine how missing data should be handled. To exclude specific values across variables that share the same stem, use the same string you passed to the `var_stem` argument as the key for identifying which values to omit. If a grouping variable is specified via the `group` argument, you can exclude particular values by using the grouping variable's name as the key to identify which values should be ignored.
...
If `group_type` is set to `variable` (the default), you can use the `margins` argument to specify how percentages are calculated and displayed in the resulting table.
...
Like the other functions in the package, you can use the `ignore` argument to specify which values to exclude from all variables associated with the provided variable stem.
...
As with `select_group_tbl()`, you can specify which values to exclude and whether to omit missing values. Be sure to use the same string provided to the `var_stem` argument to determine which values to exclude for variables sharing the same stem. You can exclude specific values from the grouping variable by using its name as the key to identify which values to omit.
