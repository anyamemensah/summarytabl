...
#'
#' @description `cat_group_tbl()` presents frequency counts and percentages (count, percent)
#' for nominal or categorical variables by some grouping variable. Relative frequencies and
#' percentages of each level of the primary categorical variable (`row_var`) within each level
#' of the grouping variable (`col_var`) can be returned. Missing data can be excluded for either
#' variable from the calculations. By default, the table is returned in the long format.
#'
#' @param data A data frame.
#' @param row_var  A character string of the name of a column in `data` containing categorical
#' data. This is the primary categorical variable. When pivoted to the `wider` format, the
#' categories of this variable will appear in the rows of the table.
#' @param col_var A character string of the name of a column in `data` containing categorical
#' data. This is the primary categorical variable. When pivoted to the `wider` format, the
#' categories of this variable will appear in the rows of the table.
...
#' should be removed before calculations. Default is `FALSE`.
#' @param ignore A named character vector or list containing values to ignore from `row_var`
#' and `col_var`.
...
#' @param only A character string or vector of strings indicating the types of summary data
#' to return. The default is `NULL`, which includes both counts and percentages. To return
#' only one type, specify `count` or `percent`. Percentages are calculated column-wise,
#' grouped by `col_var`.
#'
#' @returns A tibble displaying the relative frequency counts and/or percentages of `row_var`,
#' grouped by `col_var.`
#'
...
cat_group_tbl <- function(data,
  ...
  # Check 'pivot' is a character vector of length one and is one of 'wider' or 'longer'
  ...
  if (! (pivot %in% c("longer", "wider"))) {
    stop("Invalid 'pivot' argument. 'pivot' must be one of 'longer', 'wider'.")
  }
  ...
  if (!(all(only %in% .only_type("cat"))) || length(only) < 1){
  ...
  if (pivot == "wider") {
    cat_group_tabl <-
      cat_group_tabl |>
      tidyr::pivot_wider(id_cols = dplyr::all_of(row_var),
                         names_from = col_var,
