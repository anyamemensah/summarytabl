...
#' for nominal or categorical variables. Missing data can be excluded from the calculations.
#'
#' @param data A data frame.
#' @param var A character string of the name of a variable in `data` containing categorical
#' data.
#' @param na.rm A logical value indicating whether missing values should be removed
#' before calculations. Default is `FALSE`.
...
#' percentages. To return only counts or percentages, use `count` or
#' `percent`, respectively.
#' @param ignore An optional vector that contains values or labels you wish to exclude
#' from the data and any following calculations. Default is `NULL`
#'
#' @returns A tibble displaying the relative frequency counts and/or percentages of `var`.
#'
...
cat_tbl <- function(data, var, na.rm = FALSE, only = NULL, ignore = NULL) {
  ...
  if (is.atomic(ignore) && is.vector(ignore) && !anyNA(ignore) && length(ignore) > 0) {
    data <-
      data |>
