...
#'
#' @description `select_group_tbl()` presents frequency counts and percentages (count, percent)
#' for binary (e.g., Unselected/Selected), multiple-response (e.g., never, sometimes, often),
...
#' from the calculations in a listwise fashion.
#'
...
#' @param only A character string or vector of character strings of the kinds of summary data
#' to return. Default is `NULL`, which returns counts (count) and percentages (percent).
...
#'
#' @returns A tibble with selected frequency counts and/or percentages.
#'
...
select_group_tbl <- function(data,
  ...
  if (!(all(only %in% .only_type("select"))) || length(only) < 1){
  ...
  if (pivot == "wider") {
    select_group_tabl <-
      select_group_tabl |>
      tidyr::pivot_wider(id_cols = dplyr::all_of(c("variable","values")),
                         names_from = !!rlang::sym(group),
                         names_glue =  paste0("{.value}_",
                                              "{",ifelse(group_type == "pattern", "group", group),
                                              "}"),
