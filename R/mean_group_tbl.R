...
#'
#' @description `mean_group_tbl()` presents descriptive statistics (mean, sd, minimum, maximum, number
#' of non-missing observations) for interval (e.g., Test scores) and ratio level (e.g., Age)
#' variables with the same variable stem by some grouping variable. A variable stem is a common prefix
#' found in related variable names, often corresponding to similar survey items, that represents a
#' shared concept before unique identifiers (like timepoints) are added. For example, in the
#' `stem_social_psych` dataset, the two variables 'belong_belongStem_w1' and 'belong_belongStem_w2'
#' share the variable stem 'belong_belongStem' (e.g., "I feel like an outsider in STEM"), with suffixes
#' (_w1, _w2) indicating different measurement waves. By default, missing data are excluded from the
#' calculations in a listwise fashion.
#'
#' @param data A data frame.
#' @param var_stem A character string of a variable stem or the full name of a variable in `data`.
...
#' @param group A character string of a variable in `data` or a pattern to use to search for variables
#' in `data`.
#' @param group_type A character string that defines the type of grouping variable. Should be one of
#' `pattern` or `variable`. Default is `variable`, in which case the variable matching the `group`
#' string will be searched for within `data`.
...
#' Default is `FALSE`.
#' @param remove_group_non_alnum A logical value indicating whether to remove all non-alphanumeric
#' characters (anything that is not a letter or number) from `group`. Default is `TRUE`.
...
#'
#' @returns A table presenting summary statistics for series of continuous variables with
#' the same variable stem, categorized by a grouping variable.
#'
...
#'                               belong_welcomedStem_w1 = "I feel welcomed in STEM workplaces",
#'                               belong_welcomedStem_w2 = "I feel welcomed in STEM workplaces"),
#'                group_name = "wave")
#'
#' grouped_data <-
...
mean_group_tbl <- function(data,
  ...
  # If 'group_type' is 'pattern', verify that the count of unique columns,
  # excluding the grouping pattern, does not exactly equal elements in 'cols'
  # and has a length of at least one.
  if (group_type == "pattern") {
    cols_no_group <- unique(gsub(pattern = group, replacement = "", x = cols))

    if (all(cols_no_group %in% cols)) {
      stop(paste0(
  ...
  # Check 'na_removal' is a character vector of length one, and is one of 'listwise' or 'pairwise'
  ...
  if (!(all(only %in% .only_type("mean"))) || length(only) < 1){
  ...
  if (!is.null(ignore) && is.vector(ignore) && length(ignore) > 0) {
    ...
    if (group %in% names(ignore) && group_type == "variable") {
      group_ignore <- purrr::map(group, ~ ignore[[group]]) |> stats::setNames(group)
    } else {
      group_ignore <- NULL
    }
    ...
    data <-
      ...
      dplyr::mutate(
        dplyr::across(
          ...
          ~ dplyr::case_when(.x %in% all_ignore[[dplyr::cur_column()]] ~ NA, .default = .x)
        ))
  }
  ...
  if (na_removal == "listwise") {
    data <- stats::na.omit(data[c(all_cols)])
  }
